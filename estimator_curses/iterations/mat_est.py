...
def display_materials(stdscr, materials, selected_row, search_term=""):
    ...
    stdscr.addstr(1, 0, "-"*75)  # Longer partition line
    stdscr.addstr(2, 0, f"{'Material':<20} | {'Price (€)':<10} | {'Available Quantity':<20} | {'Required Quantity':<20}")
    stdscr.addstr(3, 0, "-"*75)  # Longer partition line
    ...
    for idx, name in enumerate(material_names):
        ...
        if idx == selected_row:
            ...
            stdscr.addstr(row, 0, f"{name:<20} | {price_str:<10} | {details['available_quantity']:<20} | {details['required_quantity']:<20}")
...
def display_buttons(stdscr, selected_button):
    ...
    stdscr.addstr(20, 0, "-"*75)  # Longer partition line for buttons
