...
def track_yellow_object(camera_index=0):
    """
    Tracks a yellow object using a Kalman filter, and saves the path with timestamps to a CSV file.
    """
    try:
        ...
        # List to store coordinates for path drawing
        path_points_for_drawing = []

        # List to store timestamp and coordinates for saving to CSV
        path_points_for_csv = []
        ...
        while True:
            ...
            if contours:
                ...
                # Add the current position to the drawing path
                path_points_for_drawing.append(center)

                # Calculate time elapsed since the start
                elapsed_time = time.time() - start_time

                # Add timestamp and position to the list for CSV
                ...
                cv2.circle(frame, predicted_point, 5, (0, 255, 255), -1)

                # Draw the predicted path in yellow
                if len(path_points_for_drawing) > 1:
                    cv2.line(frame, path_points_for_drawing[-2], predicted_point, (0, 255, 255), 2)
