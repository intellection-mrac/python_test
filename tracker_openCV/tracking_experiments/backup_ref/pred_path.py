...
def track_yellow_object(camera_index=0):
    try:
        ...
        # List to store coordinates for path drawing (just (x, y)) for actual path and predicted path
        actual_path_points = deque(maxlen=500)  # Keep a long trail for actual path
        predicted_path_points = deque(maxlen=5)  # Keep a smaller trail for predicted path
        ...
        while True:
            ...
            if contours:
                ...
                cv2.circle(frame, center, 5, (0, 255, 255), -1)  # Actual point in yellow
                ...
                if len(actual_path_points) > 1:
                    for i in range(1, len(actual_path_points)):
                        # Light yellow color for actual path trail
                        cv2.line(frame, actual_path_points[i - 1], actual_path_points[i], (255, 255, 180), 2)  # Light yellow path for actual points

                if len(predicted_path_points) > 1:
                    for i in range(1, len(predicted_path_points)):
                        # Teal color for predicted path trail
                        cv2.line(frame, predicted_path_points[i - 1], predicted_path_points[i], (0, 128, 128), 2)  # Teal path for predicted points

                # Draw the predicted position (dot) in teal
                if predicted_path_points:
                    cv2.circle(frame, predicted_path_points[-1], 5, (0, 128, 128), -1)  # Predicted point in teal
            ...
            if key == ord('s'):
                ...
                with open('yellow_object_path_with_time.csv', 'w', newline='') as csvfile:
                    writer = csv.writer(csvfile)
                    writer.writerow(['Time (s)', 'Actual X', 'Actual Y', 'Predicted X', 'Predicted Y'])  # Write header
                    # Write both actual and predicted points
                    for i in range(len(actual_path_points)):
                        writer.writerow([elapsed_time, actual_path_points[i][0], actual_path_points[i][1], predicted_path_points[i][0], predicted_path_points[i][1]])
